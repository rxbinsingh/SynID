...
\begin{enumerate}[leftmargin=*,topsep=2pt,itemsep=1pt]
    \item \textbf{Synthetic identity formation}: Identity is initialized from the model's own generative prior via a multi-anchor ensemble, producing a robust CLIP embedding without any real images.
    \item \textbf{Closed-loop self-distillation}: A bootstrap refinement loop generates expression-diverse candidates, scores them by balanced identity and prompt similarity, and refines the identity embedding iteratively.
    \item \textbf{On-the-fly UNet adaptation}: A lightweight cross-attention adapter is trained in $\sim$100 steps on four synthetic images, injecting identity directly into the UNet's denoising process.
    \item \textbf{Dual-level identity injection}: Identity tokens are injected at both the text embedding level (coarse) and UNet cross-attention level (fine-grained).
    \item \textbf{Self-correcting drift correction}: A feedback loop measures CLIP drift between generated images and the identity embedding, fine-tuning the projector to close the gap.
\end{enumerate}
...
DreamBooth~\cite{ruiz2023dreambooth} and LoRA~\cite{hu2022lora} fine-tune diffusion model weights on 3--20 real images of a subject. These methods achieve high identity fidelity but require real photographs and significant compute (minutes to hours per subject).

\paragraph{Adapter-based approaches.}
IP-Adapter~\cite{ye2023ipadapter} trains a cross-attention adapter on $\sim$1M image-text pairs, enabling image-prompted generation at inference time. IP-Adapter FaceID extends this with ArcFace embeddings for face-specific conditioning. InstantID~\cite{wang2024instantid} combines face embeddings with ControlNet for high-fidelity identity transfer. All these methods require a real reference image at inference time and large-scale pretraining.

\paragraph{Foundation models for faces.}
Arc2Face~\cite{papantoniou2024arc2face} trains a face foundation model on 21M facial images, achieving ArcFace similarity of $\sim$0.79. It requires a real ArcFace embedding as input.

\paragraph{Key difference.}
SynID requires no real images at any stage---not for training, not for inference. The identity is entirely synthetic, initialized from the diffusion model's own prior and refined through self-distillation.
...
where $\text{CLIP}_{crop}$ encodes the center face region $(x \in [0.18w, 0.82w],\; y \in [0.03h, 0.67h])$.
...
\textbf{CLIP:} openai/clip-vit-base-patch32.
...
\begin{table}[h]
\centering
\caption{Comparison with identity-conditioning methods.}
...
Arc2Face~\cite{papantoniou2024arc2face} & --- & $\sim$0.79 & Yes & 21M \\
\textbf{SynID (ours)} & \textbf{0.969} & \textbf{0.791} & \textbf{No} & \textbf{4 syn.} \\
\bottomrule
\end{tabular}
\end{table}

$^\dagger$IP-Adapter run without pose conditioning; scores may not be directly comparable.

SynID outperforms IP-Adapter FaceID on CLIP identity by +0.115 and achieves ArcFace scores comparable to Arc2Face, while requiring zero real images and no pretraining.
...
We presented SynID, a zero-shot identity-consistent generation system requiring no real reference images at any stage. Through a closed-loop self-distillation pipeline combining multi-anchor ensemble embedding, bootstrap refinement, drift correction, and on-the-fly UNet adaptation, SynID achieves identity consistency comparable to systems trained on millions of real images, while operating entirely within the model's own generative prior. The core finding is that synthetic bootstrapping combined with on-the-fly adapter training can replace dataset-driven identity conditioning for text-described characters.
